 ...
 * para que no recarguen toda la pagina. En su lugar, hace una peticion
 * AJAX y actualiza solo el contenido de la tabla de usuarios.
 */
(function ($, Drupal) {
  'use strict';

  /**
   * Comportamiento Drupal para manejar la paginacion AJAX.
   * 
   * Los comportamientos (behaviors) de Drupal permiten adjuntar
   * funcionalidad JavaScript a elementos del DOM. Se adjuntan
   * automaticamente cuando se carga la pagina y despues de las
   * actualizaciones AJAX.
   */
  Drupal.behaviors.usuariosDiegoPager = {
    /**
     * Funcion attach que se ejecuta al cargar la pagina y tras cambios AJAX.
     * 
     * @param {object} context - El contexto DOM donde aplicar los comportamientos
     * @param {object} settings - Configuracion de Drupal para JavaScript
     */
    attach: function (context, settings) {
      // Selecciona los enlaces del paginador y les anhade evento de clic
      // El metodo 'once' asegura que el manejador se anhada una sola vez
      $('.pager__item a', context).once('ajax-pager').on('click', function (e) {
        // Evitamos que el enlace funcione normalmente
        e.preventDefault();
        
                
        // Divide la URL para obtener los parametros despues del signo ?
        // Ejemplo: si la URL es /usuarios-list?page=2, obtendra "page=2"
        var url = $(this).attr('href');
        var pageParam = url.split('page=')[1];
        var page = pageParam ? pageParam.split('&')[0] : 0;
        ...
        // Creamos un objeto con todos los parametros
        var params = {
          page: page
        };
        
        // Anhadimos los filtros si tienen algun valor
        if (searchName) {
          params.search_name = searchName;
        }
        if (searchSurname) {
          params.search_surname = searchSurname;
        }
        if (searchEmail) {
        ...
        $.ajax({
          url: '/usuarios-list', // URL directa sin funcion especial
          type: 'GET',
          data: params,
          success: function (data) {
            ...
            // Necesario para que los enlaces sigan funcionando
            // Es decir: "este contenido es nuevo, hazlo funcionar como el resto de la pagina"
            Drupal.attachBehaviors($('#user-list-wrapper')[0]);
          },
          error: function() {
