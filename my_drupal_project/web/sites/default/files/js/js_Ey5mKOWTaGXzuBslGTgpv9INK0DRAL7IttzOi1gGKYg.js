...
(function ($, Drupal) {
  ...
  Drupal.behaviors.usuariosDiegoPager = {
    ...
    attach: function (context, settings) {
      // Manejar clics en el paginador - Versión mejorada
      $('.pager__item a, .pager a, .pager-item a', context).once('ajax-pager').on('click', function (e) {
        e.preventDefault();
        ...
        if (!originalUrl) {
          ...
          // Intentar obtener el número de página del texto o atributos personalizados
          var pageText = $(this).text().trim();
          if (!isNaN(pageText) && pageText !== '') {
            // Si el texto del enlace es un número, usarlo como página
            console.log('Usando el texto del enlace como número de página:', pageText);
            this.performAjaxRequest({ page: parseInt(pageText) - 1 });
          }
          return;
        }
        ...
        if (pageMatch) {
          ...
          if (pathMatch) {
            ...
            // Última estrategia: usar el texto del enlace si es un número
            var pageText = $(this).text().trim();
            if (!isNaN(pageText) && pageText !== '') {
              page = parseInt(pageText) - 1; // Restar 1 porque en Drupal las páginas son base-0
            }
          }
        }
        ...
        this.performAjaxRequest(params);
      }.bind(this));
